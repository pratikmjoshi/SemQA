...
ownrq(X,Y,yes):-
	get_val(possess_by_each,Y,3,[name,X]);
...
partrgs(X,Y,Answer):-
	specify(Y,Z,Val),
	(( Z=nil,Answer=Val),!;
	( partrgu(X,Z),Answer="I understand")).
...
partrss(X,Y,Answer):-
	...
	( ( U=nil,Answer=Val1),!;
	  ...
	  ( partrgu(X,V),
	    ...
	    add_val(elements,Z,3,[name,U]),Answer="I understand" ) ).
...
partrnuq(X,Y,Alpha,Answer):-
	...
	( get_val(superpart_of_each,X,1,[Z,number,N]),
	  ...
	  strprint(["How many",X,"per",Z],Answer),Alpha=0 ).
partrnuq(X,Y,Alpha,Answer):-
	( get_val(member,Y,3,[name,U]);
	  equivq(Y,Y1,yes),
	  get_val(subpart,Y1,3,[name,U]) ),
...
% Knowledge base for sample Peter Clark paper dialog

% To specify that every element of set X is a conductor of some element in set Y
conductr(X,Y):-
	add_val(conductor_of_each,Y,3,[name,X]),
	add_val(conducted_by_each,X,3,[name,Y]).
	
% To specify that there are N elements of set X which are parts of every element of set Y
conductrn(X,Y,N):-
	conductr(X,Y),
	add_val(conductor_of_each,Y,1,[X,conductivity,N]),
	add_val(conducted_by_each,X,1,[Y,conductivity,N]).
	
% query which element X is the best conductor of Y 
best_conductorq([X|L],Y,[Z|L2],Answer):-
	( ( get_val(conductor_of_each,Y,3,[name,X]),
	    get_val(conductor_of_each,Y,1,[X,conductivity,Z]),Answer = X );
	  ( setrq(X,U,yes),not(X=U),
	    get_val(conductor_of_each,Y,3,[name,U]),
	    get_val(conductor_of_each,Y,1,[U,conductivity,Z]),Answer = X );
	  ( best_conductorq(L,Y,[Z|L2],Answer1),Answer=Answer1) );
	( best_conductorq([X|L],Y,L2,Answer1),Answer=Answer1);
	( Answer = "There are no conductors").


% To specify that set X has property Y
property(X,Y):-
	add_val(contain_property,Y,2,[name,X]),
	add_val(property_of,X,2,[name,Y]).

% query the property list of set X
property_listq(X,Vals,Answer):- 
	get_all_val(property_of,X,2,Vals1),
	( ( setrq(X,U,yes),
	    not(U=X),
	    property_listq(U,Vals2,_),
	    concat(Vals1,Vals2,Vals) );
	  ( Vals = Vals1) ),
	listprint(Vals,Answer).
